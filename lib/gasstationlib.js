...
module.exports = (config) => {
	...
	utility.makeGastankParametersHash = function(tokenaddress, gastankaddress, tokenAmount, gasAmount, valid_until) {
	...
	utility.signGastankParameters = function(tokenaddress, gastankaddress, tokenAmount, gasAmount, valid_until, privatekey) {
		const hash = utility.makeGastankParametersHash(tokenaddress, gastankaddress, tokenAmount, gasAmount, valid_until);
	...
	// signed gas TX
	utility.getGasTx = function(to, value, from_pk) {
		return new Promise((resolve, reject) => {
			Promise.all([
				web3.eth.getGasPrice(),
				web3.eth.getTransactionCount(from),
